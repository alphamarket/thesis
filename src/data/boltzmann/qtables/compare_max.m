function compare_max()
    ...
    for e_ = 1:size(envs_, 2)
        ...
        open_hidden_fig()
        plot(max(data)', 'LineWidth', 2)
        legend(legend_)
        title(sprintf('The average Q-Table''s maximum values over %s', envs_{e_}));
        xlabel('STATE/ACTION');
        ylabel('Q VALUE MAXIMUM');
        grid
        savefig(sprintf('%s-qtable-max-compare.fig', envs_{e_}))
        print(sprintf('%s-qtable-max-compare.png', envs_{e_}), '-r300', '-dpng')
    end
