...
def confirm(string):
    while True:
        q = raw_input(string.strip() + " [y/N] ").lower()
...
def arm_and_takeoff(aTargetAltitude):
    ...
    while True:
        print " Position: ", vehicle.location.global_relative_frame, [is_armable(), vehicle.armed]
...
if(confirm("Takeoff to altitude [%.2f]?" %alt)):
    arm_and_takeoff(alt)
    print "Press CTRL+C to command LAND."
